...
import com.github.zhzhair.stepscount.common.constant.TableMetaInfo;
import com.github.zhzhair.stepscount.step.mapper.CommonMapper;
import com.github.zhzhair.stepscount.step.documents.*;
...
public class StepServiceImpl implements StepService {
    ...
    public void dropTables() {
        IntStream.range(0, TableMetaInfo.tableCount).parallel().forEach(i -> commonMapper.dropTableIfExists(prefix + i));
    }
    ...
    public void uploadStep() {
        ...
        Integer count = commonMapper.getStepCount(prefix + userId % TableMetaInfo.tableCount, userId);
        if (count != null) {
            commonMapper.updateSteps(prefix + userId % TableMetaInfo.tableCount, userId, count + stepCount);
        } else {
            commonMapper.insertTables(prefix + userId % TableMetaInfo.tableCount, userId, stepCount);
        }
    ...
    @Override
    public void recordTopAll() {
        mongoTemplate.dropCollection(StepsTop.class);
        IntStream.range(0, TableMetaInfo.tableCount).parallel().forEach(i -> mongoTemplate.insertAll(commonMapper.getTopStep(prefix + i)));
